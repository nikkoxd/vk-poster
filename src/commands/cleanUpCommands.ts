  ...
  public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    if (process.env.NODE_ENV == "development") {
      rest.put(Routes.applicationCommands(process.env.CLIENT_ID as string), { body: [] })
        .then(() => interaction.reply({ content: 'Все команды были удалены. Перезапустите бота чтобы обновить команды.', ephemeral: true }))
        .catch(this.container.logger.error);
    } else {
      interaction.reply({ content: 'Для выполнения этой команды требуется среда \`development\`', ephemeral: true });
    }
  }
}